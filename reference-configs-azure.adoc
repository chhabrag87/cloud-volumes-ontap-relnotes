...
. These VMs can support running on top of managed disks. 

. These VM types are only supported for shared managed disk deployments. All new HA deployments used shared managed disks. Previously, page blobs were used.
...
HA pairs use managed disks. (Page blobs are supported with HA pairs deployed before the 9.12.1 release.) The following disk sizes are supported:
